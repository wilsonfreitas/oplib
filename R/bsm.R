...
bsmprice <- function(type, spot, strike, time, rate, yield, sigma) {
  ...
  with(df, {
    d1 <- (log(spot / strike) +
      (rate - yield + sigma * sigma / 2) * time) / (sigma * sqrt(time))
...
bsmgreeks <- function(greeks = c("delta", "gamma", "vega", "rho", "theta"),
  ...
  with(df, {
    if (grks == "delta") bsmdelta(type, spot, strike, time, rate, yield, sigma)
    if (grks == "gamma") bsmgamma(type, spot, strike, time, rate, yield, sigma)
    if (grks == "vega") bsmvega(type, spot, strike, time, rate, yield, sigma)
    if (grks == "rho") bsmrho(type, spot, strike, time, rate, yield, sigma)
    if (grks == "theta") bsmtheta(type, spot, strike, time, rate, yield, sigma)
  })
}
...
bsmdelta <- function(type, spot, strike, time, rate, yield, sigma) {
  d1 <- (log(spot / strike) + (rate - yield + sigma * sigma / 2) * time) /
    (sigma * sqrt(time))
